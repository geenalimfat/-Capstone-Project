...
//        tableView.tableHeaderView = UIView()
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if searchText.isEmpty {
            ...
            filteredTasks = tasks.filter { task in
                return task.title.lowercased().contains(searchText.lowercased()) || task.note?.lowercased().contains(searchText.lowercased()) ?? false
            }
        }
    ...
    private func refreshTasks() {
        ...
        tasks = Task.getTasks()
        // 2.
        var unfinishedTasks = tasks.filter { !$0.isComplete }
        var finishedTasks = tasks.filter { $0.isComplete }

        // Sort the tasks
        unfinishedTasks.sort { lhs, rhs in
